...
async function main() {
  ...
  assert.match(pageSrc, /flex min-w-0 flex-col items-center text-center/);
  assert.doesNotMatch(pageSrc, /Powered by \{BRAND_NAME\}/);
  assert.doesNotMatch(pageSrc, /auth\(|requireRole/);
  const logoCmp = read("app/components/partner/PartnerSharePageLogo.tsx");
  assert.match(logoCmp, /object-contain/);
  assert.match(logoCmp, /justify-center/);
  assert.match(logoCmp, /items-center/);
  assert.match(logoCmp, /w-fit/);
  assert.match(logoCmp, /max-w-\[170px\]/);
  assert.match(logoCmp, /max-h-\[70px\]/);
  assert.match(logoCmp, /onError/);
  assert.match(logoCmp, /BRAND_LOGO_LIGHT_PUBLIC_PATH/);
  assert.doesNotMatch(logoCmp, /object-cover|aspect-square/);
  assert.doesNotMatch(logoCmp, /sm:max-w-\[180px\]/);
  assert.doesNotMatch(logoCmp, /from ["']@\/app\/lib\/db["']/);
  assert.doesNotMatch(logoCmp, /uploadPartnerShareLogo|prisma\./);
  assert.match(pageSrc, /font-bold/);
  assert.match(pageSrc, /line-clamp-2/);
  assert.match(pageSrc, /overflow-wrap:anywhere/);
