...
function main() {
  ...
  console.log("1) No schema/migration for this phase");
  ...
  console.log("6) No money / gateway / provider refund movement");
  assert.doesNotMatch(notify, /REFUND_CREDIT|requestRefund\(|executeCreditCheckout/);
  ...
  assert.doesNotMatch(notify, /COMPLETED|refund\.email_completed/);
  assert.doesNotMatch(constants, /EMAIL_COMPLETED|email_completed/);
  console.log("   ok");
