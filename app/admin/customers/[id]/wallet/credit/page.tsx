...
import AdminWalletCreditForm from "@/app/components/admin/AdminWalletCreditForm";
...
export default async function AdminCustomerWalletCreditPage({
  params,
}: {
  ...
  if (!summary.accountActive) {
    return (
      <div className="space-y-6">
        ...
        <h1 className="text-2xl font-bold tracking-tight">Add wallet credit</h1>
        <div
          className="rounded-2xl border border-[var(--border)] bg-[var(--surface-2)] px-5 py-8"
          role="status"
        >
          <p className="text-sm font-medium text-[var(--heading)]">
            Credits cannot be issued to deleted customer accounts.
          </p>
        </div>
      </div>
    );
    ...
    <div className="mx-auto max-w-xl space-y-8">
      <div>
        ...
        <p className="mt-2 text-sm text-[var(--text-muted)]">
          ADMIN-only manual USD credit. This updates the customer wallet ledger
          immediately.
        </p>
      </div>
