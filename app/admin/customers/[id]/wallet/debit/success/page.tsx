...
import { getAdminCompletedWalletDebit } from "@/app/lib/admin/wallet";
...
export default async function AdminCustomerWalletDebitSuccessPage({
  params,
  searchParams,
}: {
  ...
  const debit = await getAdminCompletedWalletDebit(id, transactionId);
  if (!debit) {
    notFound();
  }
  ...
  return (
    <div className="mx-auto max-w-xl space-y-8">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Debit completed</h1>
      ...
      <div className="flex flex-col gap-3 text-sm sm:flex-row sm:flex-wrap">
        <Link
          href={`/admin/customers/${encodeURIComponent(debit.customerId)}`}
